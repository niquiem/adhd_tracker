...
class TestUser(unittest.TestCase):
    ...
    def test_add_duplicate_habit(self):
        ...
        with self.assertRaises(ValueError):
            self.user.add_habit(habit)  # Trying to add the same habit again
    ...
    def test_remove_nonexistent_habit(self):
        habit = Habit("Practice Quidditch", "daily")
        self.user.add_habit(habit)
        self.user.remove_habit("Nonexistent Habit")
        self.assertIn(habit, self.user.habits)
    ...
    @patch('user.load_habits_from_db', return_value=[])
    def test_load_habits_empty(self, mock_load_habits_from_db):
        user = User(username="TestUser", user_id=2)
        
        self.assertEqual(user.habits, [])
